...
def callbacks(JointTrajectoryControllerState):
	...
	if  desired[0] - 0.05 <= actual[0] <= desired[0] + 0.05 and desired[1] - 0.11 <= actual[1] <= desired[1] + 0.11 and desired[2] - 0.11 <= actual[2] <= desired[2] + 0.11 and desired[3] - 0.11 <= actual[3] <= desired[3] + 0.11 and desired[4] - 0.25 <= actual[4] <= desired[4] + 0.25 and desired[5] - 0.11 <= actual[5] <= desired[5] + 0.11 and desired[6] - 0.15 <= actual[6] <= desired[6] + 0.15 and bypass2 ==1: 
	...
	#The condition of these loop will be satisfied when the arm will atteign the desired postion
	if  desired[0] - 0.05 <= actual[0] <= desired[0] + 0.05 and 2.355 - 0.11 <= actual[1] <= 2.355 + 0.11 and desired[2] - 0.11 <= actual[2] <= desired[2] + 0.11 and desired[3] - 0.11 <= actual[3] <= desired[3] + 0.11 and desired[4] - 0.11 <= actual[4] <= desired[4] + 0.11 and desired[5] - 0.11 <= actual[5] <= desired[5] + 0.11 and desired[6] - 0.15 <= actual[6] <= desired[6] + 0.15 and bypass3 == 1:

		print('PHASE III')
		#Move the robot too a position
	...
	if  desired[0] - 0.15 <= actual[0] <= desired[0] + 0.15 and desired[1] - 0.2 <= actual[1] <= desired[1] + 0.2 and desired[2] - 0.2 <= actual[2] <= desired[2] + 0.2 and desired[3] - 0.2 <= actual[3] <= desired[3] + 0.11 and desired[4] - 0.2 <= actual[4] <= desired[4] + 0.2 and desired[5] - 0.2 <= actual[5] <= desired[5] + 0.2 and desired[6] - 0.2 <= actual[6] <= desired[6] + 0.2 and bypass4 == 1:
	
		print('PHASE IV')
		moveJoint ([0.0,3.14,0.0,1.57,0.0,3.14,0.0])
		moveRobot(secndpos)
		#Disable the loop
		bypass4 = 0
##########################################################################################################
...
def callback(Odometry):
	...
	px = robot.pose.pose.position.x
	py = robot.pose.pose.position.y	
	pz = robot.pose.pose.position.z
		
	ox = robot.pose.pose.orientation.x
	oy = robot.pose.pose.orientation.y
	oz = robot.pose.pose.orientation.z
	ow = robot.pose.pose.orientation.w
...
def checkpos():
	...
	rospy.spin()
##########################################################################################################
#Take the argument of the python file 
def posrobotarg():
	coord = []
...
def convertquater(quaternions):
	...
	#Convert the quaternions angle in euler angle system 
	q0 = quaternions[6] 
	q1 = quaternions[3]
	q2 = quaternions[4]
	q3 = quaternions[5]
...
if __name__ == '__main__':
  try:  
    ...
    arm_od1 = [0.0,2.355,0.0,3.925,0.0,3.14,1.57]
    arm_od2 = [0.0,1.57,0.0,3.14,0.0,3.14,1.57]
